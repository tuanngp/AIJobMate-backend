...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Chạy Authentication Service")
    parser.add_argument("--host", default="127.0.0.1", help="Host để bind server")
    parser.add_argument("--port", type=int, default=8001, help="Port để bind server")
    ...
    logger.info(f"Chạy Authentication Service tại http://{args.host}:{args.port}")
    logger.info("Truy cập http://localhost:8001/docs để xem Swagger UI API documentation")
